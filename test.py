...
FILENAME = "/media/mobs/DimaERC2/TEST1_Basile/M1199_20210408_UMaze.xml"
nameExp = 'TEST'
ProjectPath = Project(os.path.expanduser(FILENAME), nameExp='TEST')
folderCode = "/home/mobs/Dropbox/Mobs_member/BasileToulemonde/Code/full_neuroEncoder/neuroEncoders-master"
    ...
    if not test1 :
        ...
        if singleSpike:
            ...
            subprocess.run([os.path.join(codepath, "executeFilter_stride.sh"),
                            ...
                            str(BUFFERSIZE),
                            str(windowSize),
                            str(0.200)]) #the striding is 36ms based...
...
Linearizer.verify_linearization(DataHelper.positions/DataHelper.maxPos(),
...
NNTrainer.train(DataHelper.fullBehavior, windowsizeMS=200,
                    isPredLoss=isPL)
NNTrainer.test(DataHelper.fullBehavior, l_function=l_function,
                                    windowsizeMS=200, isPredLoss=isPL)
print_results.print_results(NNTrainer.folderResult, windowSizeMS=200)
# Create json

modelPath = os.path.join(NNTrainer.folderModels, str(0.200),
