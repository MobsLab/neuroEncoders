...
class Trainer():
	...
	def train(self, behavior_data, cluster_data, speed_cut=0, masking_factor=20, rand_param=0):
		...
		for tetrode in tqdm(range(n_tetrodes)):
			# Get cluster for inferring, verify in test epochs
			# as well as that the closest measure position is taken into account.
			# guessed_clusters_time.append(cluster_data['Spike_times'][tetrode][reduce(np.intersect1d,
			# 		(np.where(behavior_data['Times']['speedFilter'][cluster_data['Spike_pos_index'][tetrode][:,0]]),
			# 	 	inEpochs(cluster_data['Spike_times'][tetrode][:,0],behavior_data['Times']['testEpochs'])))])
			# guessed_clusters.append(butils.shuffle_labels(cluster_data['Spike_labels'][tetrode][reduce(np.intersect1d,
			# 		(np.where(behavior_data['Times']['speedFilter'][cluster_data['Spike_pos_index'][tetrode][:,0]]),
			# 	 	inEpochs(cluster_data['Spike_times'][tetrode][:,0],behavior_data['Times']['testEpochs'])))], rand_param))

			guessed_clusters_time.append(cluster_data['Spike_times'][tetrode][
				 	inEpochs(cluster_data['Spike_times'][tetrode][:,0],behavior_data['Times']['testEpochs'])])
			guessed_clusters.append(butils.shuffle_labels(cluster_data['Spike_labels'][tetrode][
				 	inEpochs(cluster_data['Spike_times'][tetrode][:,0],behavior_data['Times']['testEpochs'])],rand_param))
	...
	def test_Pierre(self, bayes_matrices, behavior_data, windowSize=0.036, masking_factor=20):
		...
		for bin in range(n_bins):
			...
			for tetrode in range(len(guessed_clusters)):
				...
				# because some spike might be closest to one position further away than windowSize, yet tehmselves be close to the spike time
		...
		outputResults = {"inferring": inferResults, "probaMaps": position_proba, 'nSpikes': np.array(nSpikes)}
		return outputResults
