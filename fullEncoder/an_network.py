...
class LSTMandSpikeNetwork():
    ...
    def train(self, behaviorData, onTheFlyCorrection=False, windowsizeMS=36, scheduler='decay',
        ...
        for key in checkpointPath.keys():
            ...
            self.model.save(os.path.join(self.folderModels, str(windowsizeMS),"savedModels"))

    def train_binary(self, behaviorData, onTheFlyCorrection=False, windowsizeMS=36, scheduler='decay',
                 isPredLoss=True, earlyStop=False):
        ### Create neccessary arrays
        epochMask = {}
        totMask = {}
        ...
        for key in totMask.keys():
            ...
            # This is just max normalization to use if the behavioral data have not been normalized yet
            if onTheFlyCorrection:
                maxPos = np.max(behaviorData["pos"][np.logical_not(np.isnan(np.sum(behaviorData["pos"], axis=1)))])
    ...
    def test_binary(self, behaviorData, l_function=[], windowsizeMS=36, useSpeedFilter=False,
    ...
    def testControl(self, behaviorData, modelPath, l_function=[], windowsizeMS=36, useSpeedFilter=False,
             useTrain=False, onTheFlyCorrection=False, isPredLoss=False):
        ...
        print('Loading the weights of the trained network')    
        if len(behaviorData["Times"]["lossPredSetEpochs"])>0 and isPredLoss:
            self.model.load_weights(os.path.join(modelPath, str(windowsizeMS), "predLoss" + "/cp.ckpt"))
        else:
            self.model.load_weights(os.path.join(modelPath, str(windowsizeMS), "full" + "/cp.ckpt"))
...
class LSTMandSpikeNetwork_control():
    ...
    def testControl(self, behaviorData, wdata, l_function=[], windowsizeMS=36, useSpeedFilter=False,
             useTrain=False, onTheFlyCorrection=False, isPredLoss=False):
