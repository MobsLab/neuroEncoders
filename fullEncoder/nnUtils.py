...
def get_spike_sequences(params, generator):
	...
	for pos_index, grp, time, spike, pos in generator:
		...
		if time > windowStart + params.windowLength:
			...
			allSpikes = [np.zeros([0, params.nChannelsPerGroup[g], 32]) \
...
def parse_serialized_sequence(params, tensors, batched=False): #featDesc, ex_proto,
	...
	for g in range(params.nGroups):
		...
		if batched:
			tensors["group"+str(g)] = tf.reshape(tensors["group"+str(g)], [params.batchSize, -1, params.nChannelsPerGroup[g], 32])
		# even if batched: gather all together
		tensors["group"+str(g)] = tf.reshape(tensors["group"+str(g)], [-1, params.nChannelsPerGroup[g], 32])
		...
		nonZeros  = tf.logical_not(tf.equal(tf.reduce_sum(input_tensor=tf.cast(tf.equal(
			tensors["group"+str(g)], zeros), tf.int32), axis=[1,2]), 32*params.nChannelsPerGroup[g]))
