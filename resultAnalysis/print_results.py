...
def print_results(dir, show=False, typeDec='NN', euclidean=False, results=[],
	...
	if typeDec == 'NN':
		...
		pos = pd.read_csv(os.path.expanduser(os.path.join(dir, str(windowSizeMS), 'featureTrue.csv'))).values[:,1:]
		inferring = pd.read_csv(os.path.expanduser(os.path.join(dir, str(windowSizeMS), 'featurePred.csv'))).values[:,1:]
		qControl = np.squeeze(pd.read_csv(os.path.expanduser(os.path.join(dir, str(windowSizeMS), 'lossPred.csv'))).values[:,1:])
		if os.path.isfile(os.path.expanduser(os.path.join(dir, str(windowSizeMS), 'linearPred.csv'))):
			linear = True
			lpos = np.squeeze(pd.read_csv(os.path.expanduser(os.path.join(dir, str(windowSizeMS), 'linearTrue.csv'))).values[:,1:])
			linferring = np.squeeze(pd.read_csv(os.path.expanduser(os.path.join(dir, str(windowSizeMS), 'linearPred.csv'))).values[:,1:])
		else:
			linear = False
	elif typeDec == 'bayes':
		predLossName = 'proba'
	...
	print("total windows:",len(temp),"| selected windows:",len(frames), "(thresh",thresh,")")
	# Calculate 1d and 2d errors
	error = np.array([np.linalg.norm(inferring[i,:] - pos[i,:]) for i in range(inferring.shape[0])]) # eurledian distance
	print('mean eucl. error:', np.nanmean(error)*maxPos, "| selected error:", np.nanmean(error[frames])*maxPos)
	if linear:
		lError = np.array([np.abs((linferring[n] - lpos[n])) for n in range(linferring.shape[0])])
		print('mean linear error:', np.nanmean(lError), "| selected error:", np.nanmean(lError[frames])*maxPos)
	sys.stdout.write("threshold value: "+str(thresh)+"\r"); sys.stdout.flush()
	# Plot things
	overview_fig(pos, inferring, selection, outdir, dimOutput=dimOutput, show=block, typeDec=typeDec)
	fig_interror(pos, error, qControl, selection, thresh, outdir, dimOutput=dimOutput, show=block, typeDec=typeDec)
	if linear:
		overview_fig(lpos, linferring, selection, outdir, dimOutput=1, show=block, typeDec=typeDec)
		fig_interror(lpos, lError, qControl, selection, thresh, outdir, dimOutput=1, show=block, typeDec=typeDec)


	### Figures
	# Overview
def overview_fig(pos, inferring, selection,  outfolder, dimOutput=2, show=False, typeDec='NN'):	
	if dimOutput == 2:
		...
		ax2.plot(np.where(selection)[0], inferring[selection], label='guessed linear selection')
		ax2.plot(pos, label='true linear', color='xkcd:dark pink')
		ax2.legend()
		ax2.set_title('linear position')
	plt.savefig(os.path.expanduser(os.path.join(outfolder, f'overviewFig_{dimOutput}d_{typeDec}.png')), bbox_inches='tight')
	if show:
		plt.show(block=show)
	print()

	# Interactive figure 2D
def fig_interror(pos, Error, q_control, selection, thresh, outfolder, dimOutput=2, show=False, typeDec='NN'):
	fig, ax = plt.subplots(figsize=(15,9))
	...
	if dimOutput == 2:
		s = plt.scatter(pos[selection,0],pos[selection,1], c=Error[selection], s=10) # X and Y
		plt.axis('scaled')
	elif dimOutput == 1:
		s = plt.scatter(np.where(selection)[0],pos[selection], c=Error[selection], s=10)	
	...
	slider = Slider(
		axfreq, 'selection', 
		np.nanmin(q_control[np.isfinite(q_control)]), np.nanmax(q_control[np.isfinite(q_control)]), 
		valinit=thresh, 
		valstep=(np.nanmax(q_control[np.isfinite(q_control)]) - np.nanmin(q_control[np.isfinite(q_control)]))/100)
	...
	scatt = plt.scatter(Error[selNoNans], q_control[selNoNans], c=z, s=10, cmap=plt.cm.get_cmap('Greys'), vmin=0, vmax=1)
	...
	for bin in range(nBins):
		...
		histIdx.append(temp)
	err=np.array([
		[np.median(q_control[histIdx[n]])-np.percentile(q_control[histIdx[n]],30) for n in range(nBins) if len(histIdx[n])>10],
		[np.percentile(q_control[histIdx[n]],70)-np.median(q_control[histIdx[n]]) for n in range(nBins) if len(histIdx[n])>10]])
	plt.errorbar(
		[(edges[n+1]+edges[n])/2 for n in range(nBins) if len(histIdx[n])>10],
		[np.median(q_control[histIdx[n]]) for n in range(nBins) if len(histIdx[n])>10], c='xkcd:cherry red', 
		yerr = err, 
	...
	coefs = poly.polyfit(
		[(edges[n+1]+edges[n])/2 for n in range(nBins) if len(histIdx[n])>10], 
		[np.median(q_control[histIdx[n]]) for n in range(nBins) if len(histIdx[n])>10], 
		2)
	...
	plt.savefig(os.path.expanduser(os.path.join(outfolder, f'errorFig_{dimOutput}d_{typeDec}.png')), bbox_inches='tight')
	if show:
		plt.show(block=show)
	print()
