...
def generate_json(projectPath, modelPath ,listChannels, useOpenEphysFilter=False, offline=False):
    ...
    thresholdsParsed = [[float(s) for s in t.split("Float32[")[1].split("]")[0].split(" ")] 
                        for t in thresholdsJulia[:,1]]
    thresholds = [[] for _ in range(len(listChannels))]
    for i,j in enumerate(thresholdsJulia[:,0]):
        if offline:
            thresholds[j-1] = [t/0.195 for t in thresholdsParsed[i]] ## in offline mode the data
        else:
    ...
    for group in range(len(listChannels)):
        ...
        for chnl in range(len(listChannels[group])):
            outjsonStr['group' + str(group - idx)]['channel' + str(chnl)] = int(listChannels[group][chnl])
    ...
    if not offline:
        ...
        for e in range(nChannels):
            ...
            newDict["continuous"][0]["channels"] = newDict["continuous"][0]["channels"]+[newExampleChannel]
        for i in range(4):
