...
function extract_spike_with_buffer(xmlPath,datPath,behavePath,fileName,datasetName,datasetNameSleep,BUFFERSIZE)
	#Read the xml file:
	xdoc = parse_file(xmlPath)
	xroot = root(xdoc)
	acquiSystem =  xroot["acquisitionSystem"]
	...
	buffer_mmap = zeros(Int16,BUFFERSIZE+15+16,size(mmapFile,2))
	...
	for idBuff in ProgressBar(1:1:lastBuffIndex)
		...
		buffer_mmap[startid:lastid,:] = idBuff == 1 ? mmapFile[1:min(BUFFERSIZE+16,size(mmapFile,1)),:] : mmapFile[(idBuff-1)*BUFFERSIZE+16+1:min(idBuff*BUFFERSIZE+16,size(mmapFile,1)),:]
		#apply the low pass filter on the mmapFile:
		for id in (startid:1:size(buffer_mmap,1))
			state = α*state .+ β*buffer_mmap[id,channel_focus].*0.195
			buffer_mmap[id,channel_focus] .= buffer_mmap[id,channel_focus] .- Int16.(floor.(state./0.195))
		end
		#Note: to reduce the memory footprint we decide to round the filtering result to the upper integer
		...
		buffer_mmap = zeros(Int16,1)
		feats = []
		GC.gc()  #garbage collect the buffer as well as the feat disctionaries and all other arrays
		buffer_mmap = zeros(Int16,BUFFERSIZE+15+16,size(mmapFile,2))
		buffer_mmap[1:31] .= buffer_end
	end
end

extract_spike_with_buffer(xmlPath,datPath,behavePath,fileName,datasetName,datasetNameSleep,BUFFERSIZE)
