...
sys.path.append('/home/mobsrick/Dropbox/Mobs_member/Basile/Code/full_neuroEncoder/neuroEncoders-master/importData')
# Custom codes
...
def findTime(positionTime,lastBestTime,time):
	for i in range(len(positionTime)-lastBestTime-1):
		if np.abs(positionTime[lastBestTime+i]-time)<np.abs(positionTime[lastBestTime+i+1]-time):
...
def get_behavior(folder, bandwidth=None, getfilterSpeed = True, decode=False):
	...
	if np.sum(sleepPeriods) > 0: # If sleepPeriods exist
		sleepNames = ["".join([chr(c) for c in l[0][:,0]]) for l in f.root.behavior.sessionSleepNames[:,0]]
	...
	if not decode:
		...
		lEpochIndex = [[np.argmin(np.abs(positionTime-trainEpochs[2 * i + 1])),
								np.argmin(np.abs(positionTime-trainEpochs[2 * i]))] 
                          		for i in range(len(trainEpochs) // 2)]
		learningTime = [np.sum(np.multiply(speedFilter[lEpochIndex[i][1]:lEpochIndex[i][0]],
	...
	# Organize output
	behavior_data = {'Positions': positions, 'positionTime': positionTime, 'Speed': speed, 'Bandwidth': bandwidth,
...
def speed_filter(folder,overWrite=True):
	...
	with tables.open_file(folder + 'nnBehavior.mat',"a") as f:
		...
		epochToShow = []
		if IsMultiSessions:
			hab_list = [re.search('hab', sessionNames[x], re.IGNORECASE) for x in range(len(sessionNames))]
			id_hab = [x for x in range(len(hab_list)) if hab_list[x] != None]
			sleep_list = [re.search('sleep', sessionNames[x], re.IGNORECASE) for x in range(len(sessionNames))]
			id_sleep = [x for x in range(len(sleep_list)) if sleep_list[x] != None]
			if id_hab:
				for id in id_hab:
					epochToShow.extend([sessionStart[id], sessionStop[id]])
			elif id_sleep:
				id_toshow = list(range(id_sleep[0]+1, id_sleep[1])) # in between two sleeps
				for id in id_toshow:
					epochToShow.extend([sessionStart[id], sessionStop[id]])
		else:
				epochToShow.extend([myposTime[0,0], myposTime[-1,0]]) # if no sleeps, or session names, or hab take everything
		...
		l3 = ax0.scatter(timeToShow[speedFilter], np.zeros(timeToShow[speedFilter].shape[0]) - 4, c="black", s=0.2)
		ax0.set_ylabel("environmental \n variable")
		# Speed over time
		ax1 = fig.add_subplot(6, 2, (3, 4) ,sharex=ax0)
		l4, = ax1.plot(timeToShow[speedFilter], speedToshowSm[speedFilter],c="purple") # smoothed
		...
		ax2.set_xlim(np.percentile(speed_log[~np.isnan(speed_log)], 0.3), np.max(speed_log[~np.isnan(speed_log)]))
		...
		slider = plt.Slider(ax4, ' ',np.min(np.log(speedToshowSm[speedToshowSm>=0]+10**(-8))),
					  np.max(np.log(speedToshowSm[speedToshowSm>=0]+10**(-8))),valinit=speedThreshold,valstep=0.01)
		...
		def update(val):
			...
			l3.set_offsets(np.transpose(np.stack([timeToShow[speedFilter],np.zeros(timeToShow[speedFilter].shape[0]) - 4])))
		...
		plt.show()
		# Final value
		speedFilter = myspeed2 > np.exp(slider.val) # Apply the value to the whole dataset
...
def select_epochs(folder,overWrite=True):
	...
	with tables.open_file(folder + 'nnBehavior.mat',"a") as f:
		...
		if speeds.shape[0]==positionTime.shape[0]-1 :
			speeds = np.append(speeds,speeds[-1]).reshape(positionTime.shape[0],speeds.shape[1])
		...
		if IsMultiSessions:
			for k in range(len(sessionNames)):
				sessionValue[ep.inEpochs(positionTime[:,0],[sessionStart[k],sessionStop[k]])] = k
		...
		if IsMultiSessions:
			...
			if id_sleep:
				...
				epochToShow.extend([positionTime[0,0], positionTime[-1,0]]) # if no sleeps, or session names, or hab take everything
			...
			if IsMultiSessions:
				for idk,k in enumerate(id_toshow):
					if len(np.where(np.equal(sessionValue_toshow,k))[0])>0:
						ax[id].hlines(np.min(behToShow[np.logical_not(np.isnan(behToShow[:,dim])),dim])-np.std(behToShow[np.logical_not(np.isnan(behToShow[:,dim])),dim]),
									xmin=timeToShow[np.min(np.where(np.equal(sessionValue_toshow,k)))],
				...
				for dim in range(len(ls)):
					...
					if isinstance(l1, list):
						for iaxis in range(len(l1)):
							l1[iaxis].set_offsets(np.transpose(np.stack([timeToShow[ep.inEpochs(timeToShow, trainEpoch)[0]],
													behToShow[ep.inEpochs(timeToShow, trainEpoch)[0],dim]])))
							l2[iaxis].set_offsets(np.transpose(np.stack([timeToShow[ep.inEpochs(timeToShow, testEpochs)[0]],
							...
							if SetData['useLossPredTrainSet']:
								...
								try:
									...
									pass
						if SetData['useLossPredTrainSet']:
							ls[dim][2] = ax[dim].scatter(timeToShow[ep.inEpochs(timeToShow, lossPredSetEpochs)[0]],
										behToShow[ep.inEpochs(timeToShow, lossPredSetEpochs)[0], dim], c="orange", s=0.5)
					else:
						l1.set_offsets(np.transpose(np.stack([timeToShow[ep.inEpochs(timeToShow, trainEpoch)[0]],
													behToShow[ep.inEpochs(timeToShow, trainEpoch)[0],dim]])))
						l2.set_offsets(np.transpose(np.stack([timeToShow[ep.inEpochs(timeToShow, testEpochs)[0]],
												behToShow[ep.inEpochs(timeToShow, testEpochs)[0],dim]])))
						if SetData['useLossPredTrainSet']:
							try:
								ls[dim][2].remove()
...
class DataHelper:
	...
	def __init__(self, path, mode, jsonPath=None):
		...
		self.list_channels, self.samplingRate, self.nChannels = get_params(self.path.xml)
		if self.mode == 'decode':
			self.fullBehavior = get_behavior(self.path.folder, getfilterSpeed=False, decode=True)
		else:
		...
		self.stopTime  = self.positionTime[-1] # max(self.epochs["train"] + self.epochs["test"])
	...
	def maxPos(self):
		maxPos = np.max(self.positions[np.logical_not(np.isnan(np.sum(self.positions, axis=1)))])
