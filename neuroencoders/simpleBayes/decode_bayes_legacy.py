...
class Trainer:
    ...
    def train(self, behaviorData, onTheFlyCorrection=False):
        ...
        gridFeature, occupation = butils.kdenD(
            selPositions, self.bandwidth, kernel=self.kernel
        )  # 0.07s
        occupation[occupation == 0] = np.min(
            occupation[occupation != 0]
        )  # We want to avoid having zeros
        mask = occupation > (
            np.max(occupation) / self.maskingFactor
        ...
        for tetrode in tqdm(range(nTetrodes)):
            ...
            MRF = (
                ...
                * np.multiply(MRF, finalOccupation)
            ...
            for label in range(np.shape(self.clusterData["Spike_labels"][tetrode])[1]):
                ...
                if np.shape(clusterwisePos)[0] != 0:
                    ...
                    LRF = (
                        ...
                        * np.multiply(LRF, finalOccupation)
                        / behaviorData["Times"]["learning"]
                    )
        ...
        bayesMatrices = {
            ...
            "marginalRateFunctions": marginalRateFunctions,
...
class LegacyTrainer:
    ...
    def full_proba_decoding(
        self, behavior_data, bayes_matrices, timeStepPred, windowSize=36, useTrain=True
    ):
        windowSize = windowSize / 1000
