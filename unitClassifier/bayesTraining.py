...
def computeKernelDensityEstimator(inputQueue, outputQueue, OccInv, params, edges):
	while True:
		try:
			...
			nItems = pos.shape[0]
			if nItems==0:
				outputQueue.put((idx, np.ones([45,45])))
				continue
...
class Trainer():
	...
	def computeSpikeDensities(self):
		...
		for g in range(self.params.nGroups):
			inputQueue.put((
				[g], 
				np.concatenate([self.clusterPositions[g]["clu"+str(c)] for c in range(self.params.nClusters[g])]) ))
			for c in range(self.params.nClusters[g]):
				inputQueue.put((
					[g,c],
					self.clusterPositions[g]["clu"+str(c)] ))
	...
	def train(self):
		...
		with MOBSgraph.as_default():
			...
			with tf.variable_scope("bayesianDecoder"):
				...
				occMask_flat                = tf.reshape(occMask, [50*50])
			...
			with tf.Session() as sess:
				...
				for group in range(self.params.nGroups):
					...
					if group > 0:
						for op in range(len(convolutions[group])):
							convolutions[group][op].set_weights(convolutions[group-1][op].get_weights())

					n = cnt[group].eval()
					for i in trange(self.params.nEpochs*(n//self.params.batch_size)):
						# batch = next_batch(80, Data['spikes_train'][group], Data['labels_train'][group])
						# if i%50 == 0:
						# 	curr_eval = sess.run([MOBSgraph.get_tensor_by_name('group'+str(group)+'-evaluator/accuracy:0')], 
						# 							{MOBSgraph.get_tensor_by_name('group'+str(group)+'-encoder/x:0'): batch[0], 
						# 							MOBSgraph.get_tensor_by_name('group'+str(group)+'-encoder/y:0'): batch[1]})
						# 	sys.stdout.write('[%-30s] step : %d/%d, efficiency : %g' % ('='*(i*30//nSteps),i,nSteps,curr_eval[0]))
						# 	sys.stdout.write('\r')
						# 	sys.stdout.flush()

						# training step
						MOBSgraph.get_operation_by_name('group'+str(group)+'-evaluator/trainer').run() 

					# bbbb
					# final_eval, confusion = sess.run([MOBSgraph.get_tensor_by_name('group'+str(group)+'-evaluator/accuracy:0'), 
					# 								  MOBSgraph.get_tensor_by_name('group'+str(group)+'-evaluator/confusion/SparseTensorDenseAdd:0')], 
					# 								{MOBSgraph.get_tensor_by_name('group'+str(group)+'-encoder/x:0'): Data['spikes_test'][group], 
					# 								MOBSgraph.get_tensor_by_name('group'+str(group)+'-encoder/y:0'): Data['labels_test'][group]}) 
